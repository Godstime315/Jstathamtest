...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("C,mon search for yours 😁", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        await query.answer("Comrade, these are the total number of files available for your search!! I know you like it 😅\n🔰 But for better results, narrow down your search using the following examples:\n\t\t\t\t\t\t- Movie Name year\n\t\t\t\t\t\t- Eg: Uncharted 2022\n\t\t\t\t\t\t- Eg: Blonde 2022 720p\n\tⒸ Hislordship", True) 
...
async def advantage_spell_chok(msg):
    ...
    zz = await msg.reply('<b><i>Searching for your query on my database & imdb. please Wait...🧐<i/></b>')
    await asyncio.sleep(1)await msg.reply("I couldn't find anything related to that\nDid you mean any one of these?\n\nIf the movie you want is the one below, click on it"                
                    reply_markup=InlineKeyboardMarkup(btn))
